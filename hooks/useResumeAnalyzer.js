 ...
 * idle → extracting → fetching → ready | error
 */

import { useState, useCallback } from 'react';
import { parseGithubUrl }        from '../lib/pdf/parseUrl'; // browser-safe, no 'fs'
...
export default function useResumeAnalyzer() {
  ...
  const submit = useCallback(async (file) => {
    ...
    try {
      ...
      /* 2 – Find first user-profile link */
      let username = null;
      ...
      /* 3 – Fetch full profile + repo data */
      setStatus('fetching');
