/**
 * POST /api/extract
 * Accepts a multipart PDF upload and extracts GitHub links.
 *
 * Security:
 *  - Strict MIME type check (application/pdf only)
 *  - Hard file size limit (5 MB)
 *  - Temp file always cleaned up in finally
 *  - Only POST allowed
 *
 * Body:   multipart/form-data  field: "pdf"
...
export default async function handler(req, res) {
  ...
  try {
    ...
    try {
      ...
      // formidable throws on oversized files
    ...
    // Additional magic-bytes check — PDF files start with %PDF
