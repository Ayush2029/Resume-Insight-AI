 ...
 *
 * Security hardening:
 *  - Input sanitisation on all username / path params
 *  - Request timeout (8 s) to prevent hanging requests
 *  - Strict status-code handling (null on 404, throw on 403/429/5xx)
 *  - Token never logged
 */
...
/**
 * Strips anything that isn't a valid GitHub username/org character.
 * GitHub allows: A-Z a-z 0-9 hyphens (max 39 chars). No leading hyphen.
 */
...
/**
 * Sanitises a "owner/repo" full name.
 * Repo names allow: A-Z a-z 0-9 hyphen dot underscore (max 100 chars each segment).
 */
...
/**
 * Sanitises a file path — strips leading slashes and null bytes.
 */
...
/* ── Typed helpers (all inputs sanitised) ───────────────── */
