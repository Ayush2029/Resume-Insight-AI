 ...
 * lib/ai/summarize.js
 *
 * Powered by Groq (llama-3.3-70b-versatile) — free tier: 30 req/min, 1000 req/day.
 * Get your key: https://console.groq.com
 *
 * Summarisation strategy (in priority order):
 *   1. README.md  — if present and has substantial content (>120 chars)
 *   2. Deep file analysis — fetches ALL meaningful source files (up to FILE_LIMIT),
 *      builds a full understanding of the codebase, then generates summary.
 *   3. Empty repo fallback
 *
 * For deep file analysis, the prompt instructs the model to reason about:
 *   - Purpose & problem being solved
 *   - Architecture and design decisions
 *   - Tech stack with specific versions if detectable
 *   - Notable features, algorithms, or patterns
 *   - Likely use-cases / target audience
 */
...
const TIMEOUT   = 30_000; // 30 s for AI calls
...
// Files that reveal purpose and architecture — checked first
const HIGH_PRIORITY = [
...
const FILE_LIMIT       = 10;   // max files to read
const FILE_CHAR_LIMIT  = 4000; // chars per file (to fit context window)
const README_CHAR_LIMIT = 12000;

/* ── File-tree deep analysis ────────────────────────────── */
...
async function buildDeepFileContext(fullName, defaultBranch) {
  ...
  // Sort: high-priority files first, then shallow files (fewer slashes = less nested), then by extension
  const sorted = [
...
export async function summariseRepo({ readmeRaw, fullName, description, defaultBranch }) {
  ...
  /* 1 — README present and substantive */
